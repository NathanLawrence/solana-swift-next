...
class Base58SwiftTests: XCTestCase {
    ...
    public func testBase58EncodingForValidStrings() {
        for (decoded, encoded) in validStringDecodedToEncodedTuples {
            ...
            let result = Base58Tools.base58Encode(bytes)
    ...
    public func testBase58DecodingForInvalidStrings() {
        for invalidString in invalidStrings {
            let result = Base58Tools.base58Decode(invalidString)
            XCTAssertNil(result)
        }
    }
    ...
    public func testBase58CheckEncoding() {
        ...
        let actualOutput = Base58Tools.base58CheckEncode(inputData)
    ...
    public func testBase58CheckDecodingWithInvalidCharacters() {
        XCTAssertNil(Base58Tools.base58CheckDecode("0oO1lL"))
    }

    public func testBase58CheckDecodingWithInvalidChecksum() {
        XCTAssertNil(Base58Tools.base58CheckDecode("tz1Y3qqTg9HdrzZGbEjiCPmwuZ7fWVxpPtrW"))
    }

    public func testBase58TypeDoesFunction() {
        let inputData: [UInt8] = [
            6, 161, 159, 136, 34, 110, 33, 238, 14, 79, 14, 218, 133, 13, 109, 40, 194, 236, 153, 44, 61, 157, 254
        ]
        let expectedOutput = Base58Tools.base58Encode(inputData)
        let base58 = Base58(inputData)
